...
colorama.init(autoreset=True) #initiating module

#connecting to database

con= mc.connect(host="sql12.freesqldatabase.com", user= "sql12658148", password= "yNDppRdVsH", database="sql12658148")
cur= con.cursor()
cur.execute("Select * from menu;")
items = cur.fetchall()

#logo for the project that will be displayed on the top
def logo():
     os.system("cls")
...
#diplaying the options to the user and returning the desired one
def options():
...
#taking the order from the user and adding to to the orders table
def order():
    total = 0
    while True:
        logo()
...
#diplaying all the pending orders
def view():
    logo()
...
#deleting an order after it is complete
def delorder():
    logo()
...
#changing price of an item from the menu
def change():
    logo()
...
#searching an item from the menu and displaying it's details
def search():
    logo()
 ...
 #taking the input from the user and performing the desired function   
def main():        
    while True:
        logo()
        o = options()
